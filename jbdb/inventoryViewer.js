...
document.getElementById("btnSearchInvViewer").onclick = async () => {
    ...
    if (response.data.length != 0) {
        ...
        console.log("running?")
        Object.keys(dataSort).forEach((key) => {
            downloadData += `-----${key.toUpperCase()}-----\n`
            dataSort[key].forEach((value) => {
                downloadData += `${value.name} | ${key} (og owner: ${value.firstOwner})\n`
            })
        })
        ...
        document.getElementById("downloadFileInventoryViewer").href = url;
        document.getElementById("downloadFileInventoryViewer").download = `${user}'s inventory.txt`
    } else {
