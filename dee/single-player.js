...
function returnBestMove(scores) {
    let largest = {val: scores[0], index: 0};
    for (let i = 0; i < scores.length; i++) {
        if (largest.val <= scores[i]) {
            largest.val = scores[i];
            largest.index = i;
        }
    }
    console.log("computer will play: " + largest.index);
    //document.getElementById("debug").innerHTML = "I can play these moves, with the corresponding scores: " + scores
...
function beta(scores) {
    ...
    let upperBound = 5 - parseInt(difficulty);
    if (upperBound <= 0 || upperBound > choices.length) {
        upperBound = 1
    }
    return choices[randomNumber(0, upperBound)].index;
}
